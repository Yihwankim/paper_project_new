...
for features in [int(np.sqrt(24)), 8, 12, 16]:
    for number_estimator in [100, 150, 200]:
        model = RandomForestRegressor(n_estimators=number_estimator, max_features=features,
                                      criterion='mse', random_state=2)
...
rfr_outcome.to_excel('data_process/conclusion/regression_result/rfr_with_full_features_trees.xlsx')
sum_variable_important.to_excel('data_process/conclusion/regression_result/rfr_full_test_featureselection_trees_features.xlsx')
